...
def conv(inputs, kernel_size, output_num, stride_size=1, init_bias=0.0, conv_padding='SAME', stddev=0.01,
    ...
    conv_weights = tf.Variable(
        tf.random_normal([kernel_size, kernel_size, kernel_size, input_size, output_num], dtype=tf.float32, stddev=stddev),
        name='weights')
...
def fc(inputs, output_size, init_bias=0.0, activation_func=tf.nn.relu, stddev=0.01):
    ...
    if len(input_shape) == 5:
        fc_weights = tf.Variable(
            tf.random_normal([input_shape[1] * input_shape[2] * input_shape[3]* input_shape[4], output_size], dtype=tf.float32,
                             stddev=stddev),
            name='weights')
